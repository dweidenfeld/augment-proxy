...
| userAgent           | string           | "Mozilla/5.0 (Windows NT 6.1; rv:38.0) Gecko/20100101 Firefox/38.0" | Only relevant when executeJavascript is enabled. The user-agent string that is used when requesting documents from this URL pattern                                                                                                                                                                                      |
| omitContentRegex    | string           | null                                                                | A regex that is matched against the HTML content (ignored for non-HTML content) of a document from this URL pattern. When the pattern matches the AugmentProxy returns 404 for this document                                                                                                                             |
| excludeCssSelectors | array of strings | empty array                                                         | One or multiple CSS selector(s) that is/are run against a document from this URL pattern. If a CSS selector returns a node, this node is wrapped in an "googleoff: index" and "googleon: snippet" comment which causes the GSA to ignore this content (it still follows links in this content)                           |
...
| headersToMetadata   | object           | null                                                                | A simply map with a string key and a string value, mapping an HTTP header name to a metadata name. This map is used to add the HTTP headers with the respective names (from the response of a requested URL) to metadata for documents from this URL pattern                                                             |
| addRequestHeaders   | object           | null                                                                | A simply map with a string key and a string value, that is used to add additional HTTP headers to a request of documents from this URL pattern                                                                                                                                                                           |
| imageLinksToPdf     | object           | null                                                                | Explained in the next section                                                                                                                                                                                                                                                                                            |
| extract             | object           | null                                                                | Explained in the next section                                                                                                                                                                                                                                                                                            |
The imageLinksToPdf property can be used to create a very simplistic image search. When configured the AugmentProxy processes all image tags and adds the URLs of the images as new links to the document. These new links are surrounded by googleoff comments for index and snippet to not modify the actual document. When enabled ("addSurroundingText") the AugmentProxy also tries to find surrounding text around the image and combines that with the link it creates. Images can then be found with this text. The alt attribute of an image is always added as text to the link. When the GSA crawls such an image (you have to allow the respective file ending on the GSA of course), the AugmentProxy will not return an image but a PDF file (with content-type PDF). This allows the GSA to generate document previews for the image. The PDF contains only one page with the size of the image. URLs of those PDFs always contain the URL parameter "convert2pdf=true". The displayUrl is the URL of the actual image.

| Config option      | Type    | Default       | Description                                                                                                                                                                      |
|--------------------|---------|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| minWidth           | int     | 2,147,483,647 | The minimum width an image must have to be considered                                                                                                                            |
| minHeight          | int     | 2,147,483,647 | The minimum height an image must have to be considered                                                                                                                           |
| addSurroundingText | boolean | false         | Tries to find surrounding text around the image and combines it with the link it generates so that the image can be found with this text. This might not always work as intended |
...
| cssSelector     | string           | null           | The CSS selector to run on the document before applying the regexes. Only valid when "scope" is "content", ignored otherwise. The CSS selector can return multiple elements.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| regexFind       | string           | ".*"           | The regex that should be matched against the configured scope. Note that "regexMatch" configures whether the regex is matched against the HTML or text of a document (only when "scope" is "content"). When configured you should also configure "regexReplace". You can define capturing groups here which can then be used in "regexReplace".                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| regexReplace    | string           | null           | The regex that is applied when the configured "regexFind" matches. You can use capturing groups from the "regexFind" here. Note that capturing groups in Java are written like this: "$1" - this would be the first capturing group.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| metadataNames   | array of strings | empty array    | The names that should be used for the extracted metadata. Must contain at least one entry when "target" is "metadata". If "target" is something other than "metadata" configured entries are ignored. When "scope" is "url" you can only extract at most one string. Consequently when more than one metadata name is configured here, these are also ignored.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| titleCssElement | int              | 0              | The number of the matched CSS element that should be used as title of the document. Note: this setting is zero-based. So 0 will get you the first element, 1 the second and so on. When "scope" is "url" you can only extract at most one string. Consequently when "titleCssElement" is set to an number larger than 0, 0 is used.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
    ...
    [
      {
        ...
        "omitContentRegex": ".*You don't have permission to access.*",
        "excludeCssSelectors": [
          "div.header",
          "div.footer",
          "#navigation"
