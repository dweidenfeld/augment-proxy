...
##### Build notes
- The project uses the sitemap-parser, a library to process sitemaps. You can download and build that library [here](https://github.com/bithazard/sitemap-parser).
- You also need the mentioned plexi framework, which can be downloaded [here](https://github.com/googlegsa/library/releases/download/v4.1.0/adaptor-4.1.0-withlib.jar).
...
| tlsTermination      | boolean          | false                                                               | Modify all links (\<a href="..."/\>) that are contained in documents from this URL pattern and convert the protocol from https to http. That way the GSA is able to crawl them using the AugmentProxy. Internally the AugmentProxy still uses https to query documents from this URL pattern. So only the connection between the GSA and the AugmentProxy is not encrypted. |
| passRequestCookies  | boolean          | false                                                               | Take Cookies contained in the request from the GSA (add a Forms Auth rule) and pass them with the request that the AugmentProxy makes. Cookies that are set by the server in the response are also returned to the GSA.                                                                                                                                                     |
...
| excludeCssSelectors | array of strings | empty array                                                         | One or multiple CSS selector(s) that is/are run against documents from this URL pattern. If a CSS selector returns one/multiple node(s), this/these node(s) is/are wrapped in an "googleoff: index" and "googleoff: snippet" HTML comment which causes the GSA to ignore this content (it will still follow links in this content).                                         |
