...
| omitContentRegex    | string           | null                                                                | A regex that is matched against the HTML content (ignored for non-HTML content) of a document from this URL pattern. When the pattern matches, the AugmentProxy returns 404 for this document. Note that you probably want to prefix your regex with "(?s)" to match newlines with a ".".                                   |
| excludeCssSelectors | array of strings | empty array                                                         | One or multiple CSS selector(s) that is/are run against a document from this URL pattern. If a CSS selector returns a node, this node is wrapped in an "googleoff: index" and "googleon: snippet" HTML comment which causes the GSA to ignore this content (it still follows links in this content).                        |
